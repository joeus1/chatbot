...
**Hooks** (`hooks/` + `settings.json`): a tested PreToolUse guard that deterministically blocks destructive commands (`rm -rf /`-class deletes, force pushes without lease, `curl | sh`, `chmod 777`, hard resets to origin/main) and blocks writes containing real-looking secrets (OpenAI/Stripe/AWS/GitHub/Slack keys, private key blocks) or untracked-`.env`-into-git mistakes; and a SessionStart bootstrap that installs `ruff` in fresh containers, behind an atomic lock so concurrent or double-registered runs can't race, and announces the toolkit. Every hook fails open on any internal error, so none can wedge a session. (There is deliberately no auto-format hook: formatting on save rewrites files the session didn't intend to touch, and nothing here gates on formatting.)
