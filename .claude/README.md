...
**Hooks** (`hooks/` + `settings.json`): a tested PreToolUse guard that deterministically blocks destructive commands (`rm -rf /`-class deletes, force pushes without lease, `curl | sh`, `chmod 777`, hard resets to origin/main) and blocks writes containing real-looking secrets (OpenAI/Stripe/AWS/GitHub/Slack keys, private key blocks) or untracked-`.env`-into-git mistakes; a PostToolUse auto-formatter that runs `ruff format` on edited Python files; and a SessionStart bootstrap that installs `ruff` in fresh containers. Every hook fails open on any internal error, so none can wedge a session - though a missing or unreadable `setup.sh` now surfaces as a hook error rather than being swallowed, since that is a real breakage worth seeing.
