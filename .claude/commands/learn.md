...
2. Write it to `.claude/skills/<kebab-case-name>/SKILL.md` (directly under skills/ - deeper nesting is never discovered) following the continuous-learning skill's format: frontmatter with name and a trigger-rich description, then the pattern, when to use it, and a concrete example from this session (with secrets and customer data stripped).
